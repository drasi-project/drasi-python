...
## How It Works
The demo is two files in `tutorials/building-comfort/`: **`engine.py`** holds the source, the six queries, and the `ComfortEngine` (its Python reaction and the SQL writes); **`app.py`** renders the reaction's state and hosts the sidebar controls. The [complete `engine.py`](#putting-it-all-together) is at the end. Here's what each part does.
